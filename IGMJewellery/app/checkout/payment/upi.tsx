...
const upiSchema = z.object({
  upiId: z.string().min(3, 'Enter a valid UPI ID').regex(/^[\w.-]+@[\w.-]+$/, 'Invalid format'),
  remember: z.boolean(),
});
...
export default function UpiDetailsScreen() {
  ...
  const { control, handleSubmit, formState: { errors } } = useForm<UpiFormData>({
                ...
                render={({ field: { onChange, value } }) => (
                  <View>
                    <TextInput 
                      ...
                      autoCapitalize="none"
                    />
                    {errors.upiId && <Text style={styles.errorText}>{errors.upiId.message}</Text>}
                  </View>
                )}
              />
