...
const SLOTS = ['12:00 PM', '1:30 PM', '3:00 PM', '4:00 PM', '5:30 PM', '6:00 PM'];
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.heroBox}>
           ...
           <Text style={styles.heroDesc}>
              Our Try at Home consultant will visit your location to show you the designs according to your time & place
           </Text>
        </View>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
             ...
             <TouchableOpacity><Text style={styles.viewCart}>View Trial Cart</Text></TouchableOpacity>
          </View>
          <View style={styles.designGrid}>
             {[1, 2, 3].map(i => (
               <View key={i} style={styles.designPlaceholder}>
                 <Ionicons name="close" size={14} color="#000" style={styles.removeDesign} />
               </View>
             ))}
          </View>
          <Text style={styles.locationInfo}>Kalyan Jewellers  <Text style={{color: '#8E8E93'}}>400 066, Mumbai</Text></Text>
        </View>
        ...
        <View style={styles.section}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.horizontalScroll}>
             {DATES.map((d) => (
               <TouchableOpacity 
                 key={d.label} 
                 style={[styles.dateCard, selectedDate === d.label && styles.activeCard]}
                 onPress={() => setValue('date', d.label)}
               >
                 <Text style={[styles.dayText, selectedDate === d.label && styles.activeText]}>{d.day}</Text>
                 <Text style={[styles.dateText, selectedDate === d.label && styles.activeText]}>{d.date}</Text>
               </TouchableOpacity>
             ))}
          </ScrollView>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.slotsGrid}>
             {SLOTS.map((s) => (
               <TouchableOpacity 
                 key={s} 
                 style={[styles.slotItem, selectedSlot === s && styles.activeCard]}
                 onPress={() => setValue('timeSlot', s)}
               >
                 <Text style={[styles.slotText, selectedSlot === s && styles.activeText]}>{s}</Text>
               </TouchableOpacity>
             ))}
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', alignItems: 'center', padding: SPACING.m, borderBottomWidth: 1, borderBottomColor: '#F5F5F5' },
  ...
  heroBox: { alignItems: 'center', padding: 24, borderBottomWidth: 8, borderBottomColor: '#F9F9F9' },
  homeIcon: { width: 48, height: 48, borderRadius: 24, backgroundColor: '#F5F5F5', justifyContent: 'center', alignItems: 'center', marginBottom: 12 },
  heroTitle: { fontSize: 16, fontWeight: '700', marginBottom: 8 },
  heroDesc: { fontSize: 12, color: '#8E8E93', textAlign: 'center', lineHeight: 18 },
  section: { padding: SPACING.m, borderBottomWidth: 8, borderBottomColor: '#F9F9F9' },
  ...
  designPlaceholder: { width: 60, height: 60, backgroundColor: '#F5F5F5', borderRadius: 4, position: 'relative' },
  removeDesign: { position: 'absolute', top: 4, right: 4 },
  locationInfo: { fontSize: 12, fontWeight: '700' },
  horizontalScroll: { flexDirection: 'row' },
  dateCard: { width: 64, height: 72, borderWidth: 1, borderColor: '#F0F0F0', borderRadius: 8, justifyContent: 'center', alignItems: 'center', marginRight: 12, backgroundColor: '#F9F9F9' },
  ...
  slotItem: { width: '30%', height: 44, backgroundColor: '#F5F5F5', borderRadius: 6, justifyContent: 'center', alignItems: 'center' },
  slotText: { fontSize: 13, color: '#333' },
  activeCard: { borderColor: '#000', borderWidth: 1.5, backgroundColor: '#FFF' },
  activeText: { color: '#000', fontWeight: '800' },
  footerBtn: { backgroundColor: '#000', margin: 16, height: 54, borderRadius: 8, justifyContent: 'center', alignItems: 'center' },
